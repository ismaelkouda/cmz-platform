...
- **Dernière mise à jour :** 2026-08-11 (généré par `tools/generate-status.mjs`)
- **État :** **Phase 08** — génération depuis patterns ([ADR-0013](../adr/0013-phases-08-generation-et-09-verification.md)). Socle outillé + Kernel `shared/` / `@cmz/core` + **19** modules livrés/compilants (**72** libs + **1** app ; **2 722** `.ts` hors tests). Voir [`STATUS.md`](../../STATUS.md).
...
- **Bundle initial (production, raw)** : **872.78 kB** — source [`bundle-metrics.json`](../../apps/backoffice-angular/bundle-metrics.json) (mesuré 2026-08-11 via `bun run bundle:record` après build).
