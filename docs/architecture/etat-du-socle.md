...
- **Dernière mise à jour :** 2026-08-30 (généré par `tools/generate-status.mjs`)
- **État :** **Phase 08** — génération depuis patterns ([ADR-0013](../adr/0013-phases-08-generation-et-09-verification.md)). Socle outillé + Kernel `shared/` / `@cmz/core` + **21** modules livrés/compilants (**77** libs + **1** app ; **2 742** `.ts` hors tests). Voir [`STATUS.md`](../../STATUS.md).
...
- **Bundle initial (production, raw)** : **526.38 kB** — source [`bundle-metrics.json`](../../apps/backoffice-angular/bundle-metrics.json) (mesuré 2026-08-30 via `bun run bundle:record` après build).
- **ExcelJS (lazy)** : **948.37 kB** raw — hors budget initial.
