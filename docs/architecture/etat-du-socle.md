...
- **Dernière mise à jour :** 2026-09-25 (généré par `tools/generate-status.mjs`)
- **État :** **Phase 08** — génération depuis patterns ([ADR-0013](../adr/0013-phases-08-generation-et-09-verification.md)). Socle outillé + Kernel `shared/` / `@cmz/core` + **19** modules livrés/compilants (**72** libs + **3** app ; **2 730** `.ts` hors tests). Voir [`STATUS.md`](../../STATUS.md).
...
- **Bundle initial (production, raw)** : **521.22 kB** — source [`bundle-metrics.json`](../../apps/backoffice-angular/bundle-metrics.json) (mesuré 2026-09-11 via `bun run bundle:record` après build).
