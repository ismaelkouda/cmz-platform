...
| T12-24 | **Autres type-guards d'énum morts, même famille que T12-21** : `isReportStatesDetailsStatus` (`libs/report-states/domain/src/lib/enums/report-states-details-status.enum.ts`), `isProcessingDetailsState` (`libs/processing/domain/src/lib/enums/processing-details-state.enum.ts`), `isProcessingDetailsProcessingState` (`libs/processing/domain/src/lib/enums/processing-details-processing-state.enum.ts`) — vérifié via `grep -rn` (chacun ne retourne que sa déclaration + son export de barrel). Pattern probable : générateur qui scaffold systématiquement un type-guard par enum wire, alors que les mappers concernés valident via `Map.get() ?? fallback` (silencieux) plutôt que via un guard qui lèverait — contrairement à `isAccessLogsAction` (`settings-security`), lui réellement appelé et utile. Ne pas supprimer en masse sans revue — certains peuvent être une API publique voulue pour un futur appelant (ex. validation stricte côté formulaire). | ouvert | S | P2 | NOUVEAU 2026-08-10, découvert en traitant T12-21 |
