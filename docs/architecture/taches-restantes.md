...
| T12-21 | **Code mort public : `isReportStatesDetailsQualificationState`** (`libs/report-states/domain/src/lib/enums/report-states-details-qualification-state.enum.ts`), exportée depuis `report-states-domain/src/index.ts` mais jamais appelée nulle part dans le monorepo (`grep -rn` sur libs/ et apps/ ne retourne que sa déclaration et son export). Absente côté `requests` (asymétrie). Découvert en produisant le mémo P1-1. Candidat suppression si aucun consommateur externe n'est prévu — à vérifier avant suppression (peut être une API publique intentionnellement anticipée). | ouvert | S | P2 | NOUVEAU 2026-08-10, découvert en produisant P1-1 |
| T12-22 | **Même classe de fuite i18n que P1-2, prefixe différent** : `libs/report-states/ui/src/lib/constants/report-states-details-status-label.constant.ts` lignes 8-17 utilisent `REQUESTS.ALL.FILTER.STATUS_APPROVED/REJECTED/ABANDONED/IN_PROGRESS/TERMINATED/CONFIRMED` — confirmé qu'aucune clé `REPORT_STATES.ALL.FILTER.*` équivalente n'existe dans le catalogue de traduction (`grep -rn "STATUS_APPROVED" apps/backoffice-angular/src/app/i18n/fr/*.ts` ne retourne qu'une seule occurrence, sous `REQUESTS.ALL.FILTER`). Hors périmètre exact de P1-2 (qui ne couvrait que le préfixe `REQUESTS.DETAILS.`), volontairement non corrigé dans ce commit pour respecter la portée de la tâche. Découvert en exécutant P1-2 (2026-08-10). | ouvert | S | P1 | NOUVEAU 2026-08-10, découvert en exécutant P1-2 |
