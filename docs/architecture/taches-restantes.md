...
| T12-20 | **`RequestsDetailsMapper` (`libs/requests/data/src/lib/mappers/requests-details.mapper.ts`) sans aucun test unitaire** — découvert en produisant le mémo P1-1 (`docs/architecture/factorisation-details-workflow.md` §2.3). Son équivalent `ReportStatesDetailsMapper` a 18 tests dans `report-states-details.mapper.spec.ts` ; `requests-details-mappers.spec.ts` ne teste que les 4 petits mappers (filter/take/approve/reject), jamais le mapper principal. Confirmé : `grep -rln "RequestsDetailsMapper" libs/requests/ --include="*.spec.ts"` → aucun résultat. Créer `requests-details.mapper.spec.ts` sur le modèle de `report-states-details.mapper.spec.ts`. | ouvert | S | P1 | NOUVEAU 2026-08-10, découvert en produisant P1-1 |
