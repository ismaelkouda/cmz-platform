...
| J-9 | Exécution CI de `check-pattern`/`check-semantics` sur de vrais modules du dépôt | 🔧 | **`check-pattern` fait pour `crud-entity`, onzième passe (ci-dessus)** : `tools/check-pattern-nx.mjs` (nouveau, local — ne touche pas aux outils vendorés) exécuté pour de vrai contre 2 modules, 66/66 chacun, câblé en CI. `check-semantics.mjs` (9 règles de contenu) et `action-request` Nx-shaped restent non portés — effort séparé, de l'ampleur de N-7 lui-même |
