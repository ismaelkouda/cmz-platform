...
- **Décorateur** : `@Service()` — **pas** `@Injectable({ providedIn: 'root' })`.
  ...
  applique la convention Angular 22 du [profil](../../angular-22.profile.json).
...
qui lève des `domain-error` (`ServerResponseError`/`UnknownError`). Les bases
sont donc **pures** : `mapFromDto(dto) → mapItemFromDto(unwrapResponse(dto))`.
Plus de `console.log`, plus d'`ApiError` dans les mappers.
