...
import { EvaluateReportStatesUseCase } from './evaluate-report-states.use-case';

function makeEntity(uniqId: string): EvaluateReportStatesEntity {
    return new EvaluateReportStatesEntity({
...
function makePageResult(
    items: EvaluateReportStatesEntity[]
): PageResult<EvaluateReportStatesEntity> {
    return {
        items,
        currentPage: 1,
        lastPage: 1,
...
describe('EvaluateReportStatesUseCase', () => {
    it('delegates to EvaluateReportStatesRepository with validated filter and page', async () => {
        const pageResult = makePageResult([makeEntity('Q-001')]);
        const repo: EvaluateReportStatesRepository = {
            execute: vi.fn().mockReturnValue(of(pageResult)),
            export: vi.fn(),
        };
        const injector = createEnvironmentInjector(
            [
                { provide: EvaluateReportStatesRepository, useValue: repo },
                EvaluateReportStatesUseCase,
            ],
            null as never
        );
        const useCase = injector.get(EvaluateReportStatesUseCase);
