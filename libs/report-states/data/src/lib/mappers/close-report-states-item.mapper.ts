...
import type { CloseReportStatesProps } from '@cmz/report-states-domain';
import { TypeReport } from '@cmz/shared-domain';
import { CloseReportStatesItemApiDto } from '../dtos/close-report-states-response-api.dto';

@Service()
export class CloseReportStatesItemMapper extends PaginatedMapper<
...
> {
    ...
    private readonly entityCache = new Map<string, CloseReportStatesEntity>();

    protected override mapItemFromDto(
        dto: CloseReportStatesItemApiDto
    ): CloseReportStatesEntity {
        MapperUtils.validateDto(dto, { required: ['uniq_id'] });
        const props: CloseReportStatesProps = {
        ...
        const entity = cached
            ? cached.with(props)
            : new CloseReportStatesEntity(props);
