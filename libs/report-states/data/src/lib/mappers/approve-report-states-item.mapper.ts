...
import type { ApproveReportStatesProps } from '@cmz/report-states-domain';
import { TypeReport } from '@cmz/shared-domain';
import { ApproveReportStatesItemApiDto } from '../dtos/approve-report-states-response-api.dto';

@Service()
export class ApproveReportStatesItemMapper extends PaginatedMapper<
...
> {
    ...
    private readonly entityCache = new Map<string, ApproveReportStatesEntity>();

    protected override mapItemFromDto(
        dto: ApproveReportStatesItemApiDto
    ): ApproveReportStatesEntity {
        MapperUtils.validateDto(dto, { required: ['uniq_id'] });
        const props: ApproveReportStatesProps = {
        ...
        const entity = cached
            ? cached.with(props)
            : new ApproveReportStatesEntity(props);
