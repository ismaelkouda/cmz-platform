...
import type { RejectReportStatesProps } from '@cmz/report-states-domain';
import { TypeReport } from '@cmz/shared-domain';
import { RejectReportStatesItemApiDto } from '../dtos/reject-report-states-response-api.dto';

@Service()
export class RejectReportStatesItemMapper extends PaginatedMapper<
...
> {
    ...
    private readonly entityCache = new Map<string, RejectReportStatesEntity>();

    protected override mapItemFromDto(
        dto: RejectReportStatesItemApiDto
    ): RejectReportStatesEntity {
        MapperUtils.validateDto(dto, { required: ['uniq_id'] });
        const props: RejectReportStatesProps = {
        ...
        const entity = cached
            ? cached.with(props)
            : new RejectReportStatesEntity(props);
