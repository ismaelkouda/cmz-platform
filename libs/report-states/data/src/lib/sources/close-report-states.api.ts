import { HttpClient, HttpContext } from '@angular/common/http';
import { inject, Service } from '@angular/core';
import { Observable } from 'rxjs';
import { BYPASS_CACHE, REPORT_API_URL } from '@cmz/core';
import { buildHttpParams } from '@cmz/shared-data';
import { FetchOptions } from '@cmz/shared-domain';
import { CloseReportStatesResponseDto } from '../dtos/close-report-states-response-api.dto';
import { CloseReportStatesFilterApiDto } from '../dtos/close-report-states-filter-api.dto';
import { REPORT_STATES_ENDPOINTS } from '../endpoints/report-states.endpoints';

@Service()
export class CloseReportStatesApi {
    private readonly http = inject(HttpClient);
    private readonly baseUrl = inject(REPORT_API_URL);

    execute(
        filter: CloseReportStatesFilterApiDto,
        page: string,
        options?: FetchOptions
    ): Observable<CloseReportStatesResponseDto> {
        const url = `${this.baseUrl}${REPORT_STATES_ENDPOINTS.CLOSE}`;
        const context = new HttpContext().set(
            BYPASS_CACHE,
            options?.forceRefresh ?? false
        );
        const params = buildHttpParams(filter, { arrayFormat: 'comma' }).set(
            'page',
            page
        );

        return this.http.get<CloseReportStatesResponseDto>(url, {
            context,
            params,
        });
    }

    export(
        filter: CloseReportStatesFilterApiDto,
        options?: FetchOptions
    ): Observable<CloseReportStatesResponseDto> {
        const url = `${this.baseUrl}${REPORT_STATES_ENDPOINTS.CLOSE_EXPORT}`;
        const context = new HttpContext().set(
            BYPASS_CACHE,
            options?.forceRefresh ?? false
        );
        const params = buildHttpParams(filter, { arrayFormat: 'comma' });

        return this.http.get<CloseReportStatesResponseDto>(url, {
            context,
            params,
        });
    }
}
