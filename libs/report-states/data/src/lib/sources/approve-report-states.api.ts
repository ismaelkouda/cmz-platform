import { HttpClient, HttpContext } from '@angular/common/http';
import { inject, Service } from '@angular/core';
import { Observable } from 'rxjs';
import { BYPASS_CACHE, REPORT_API_URL } from '@cmz/core';
import { buildHttpParams } from '@cmz/shared-data';
import { FetchOptions } from '@cmz/shared-domain';
import { ApproveReportStatesResponseDto } from '../dtos/approve-report-states-response-api.dto';
import { ApproveReportStatesFilterApiDto } from '../dtos/approve-report-states-filter-api.dto';
import { REPORT_STATES_ENDPOINTS } from '../endpoints/report-states.endpoints';

@Service()
export class ApproveReportStatesApi {
    private readonly http = inject(HttpClient);
    private readonly baseUrl = inject(REPORT_API_URL);

    execute(
        filter: ApproveReportStatesFilterApiDto,
        page: string,
        options?: FetchOptions
    ): Observable<ApproveReportStatesResponseDto> {
        const url = `${this.baseUrl}${REPORT_STATES_ENDPOINTS.APPROVE}`;
        const context = new HttpContext().set(
            BYPASS_CACHE,
            options?.forceRefresh ?? false
        );
        const params = buildHttpParams(filter, { arrayFormat: 'comma' }).set(
            'page',
            page
        );

        return this.http.get<ApproveReportStatesResponseDto>(url, {
            context,
            params,
        });
    }

    /** Export — même filtre que la liste, sans paramètre `page`. */
    export(
        filter: ApproveReportStatesFilterApiDto,
        options?: FetchOptions
    ): Observable<ApproveReportStatesResponseDto> {
        const url = `${this.baseUrl}${REPORT_STATES_ENDPOINTS.APPROVE_EXPORT}`;
        const context = new HttpContext().set(
            BYPASS_CACHE,
            options?.forceRefresh ?? false
        );
        const params = buildHttpParams(filter, { arrayFormat: 'comma' });

        return this.http.get<ApproveReportStatesResponseDto>(url, {
            context,
            params,
        });
    }
}
