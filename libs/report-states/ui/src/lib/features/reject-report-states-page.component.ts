...
import { RejectReportStatesVmProps } from '../adapters/reject-report-states-vm-props.interface';
import { RejectReportStatesPresenter } from '../adapters/reject-report-states-vm.presenter';
import { REJECT_REPORT_STATES_FILTER_KEYS } from '../constants/reject-report-states-filter-keys.constant';
import { REPORT_STATES_REJECT_ROUTE } from '../constants/report-states-paths.constant';
import { REJECT_REPORT_STATES_TABLE } from '../constants/reject-report-states-table.constant';
import { RejectReportStatesFilterStore } from '../stores/reject-report-states-filter.store';
import {
...
@Component({
    selector: 'cmz-reject-report-states-page',
    imports: [
        FilterComponent,
        TableComponent,
        PaginationComponent,
    ...
    providers: [RejectReportStatesFilterStore],
    template: `
...
export class RejectReportStatesPageComponent {
    protected readonly facade = inject(RejectReportStatesFacade);
    ...
    protected readonly tableColumns = REJECT_REPORT_STATES_TABLE.cols;
    protected readonly tableActions = REJECT_REPORT_STATES_TABLE.actions;
    ...
    private readonly presenter = new RejectReportStatesPresenter((key) =>
        this.i18n.translate(key)
    );

    protected readonly itemsVM = computed<RejectReportStatesVmProps[]>(() =>
        this.facade
            .items()
            .map((item) =>
    ...
    protected readonly filterFields: Signal<FilterField[]> = computed(() => [
        {
            type: 'text',
            name: REJECT_REPORT_STATES_FILTER_KEYS.INITIATOR_PHONE_NUMBER,
            label: `${T}.FILTER.INITIATOR`,
            placeholder: 'COMMON.PHONE_PLACEHOLDER',
        },
        {
            type: 'text',
            name: REJECT_REPORT_STATES_FILTER_KEYS.UNIQ_ID,
            label: `${T}.FILTER.UNIQ_ID`,
            placeholder: 'COMMON.REPORT_UNIQ_ID_PLACEHOLDER',
        },
        {
            type: 'select',
            name: REJECT_REPORT_STATES_FILTER_KEYS.REPORT_TYPE,
        ...
        {
            type: 'text',
            name: REJECT_REPORT_STATES_FILTER_KEYS.OPERATORS,
            label: `${T}.FILTER.OPERATORS`,
            placeholder: 'COMMON.SELECT_PLACEHOLDER',
        },
        {
            type: 'select',
            name: REJECT_REPORT_STATES_FILTER_KEYS.SOURCE,
        ...
        {
            type: 'date',
            name: REJECT_REPORT_STATES_FILTER_KEYS.START_DATE,
            label: 'COMMON.START_DATE',
        },
        {
            type: 'date',
            name: REJECT_REPORT_STATES_FILTER_KEYS.END_DATE,
