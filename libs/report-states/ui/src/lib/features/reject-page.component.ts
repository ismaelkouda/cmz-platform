...
import { RejectFacade } from '@cmz/report-states-application';

@Component({
    selector: 'cmz-reject-page',
    standalone: true,
    ...
    template: `
        ...
        >
            <div class="flex items-center justify-between mb-6">
                <div>
                    <h2 class="text-xl font-bold text-slate-100">
                        Demandes Non Recevables (Rejetées)
                    </h2>
                    <p class="text-sm text-slate-400 mt-1">
                        Liste des signalements rejetés ou déclarés non
                        recevables.
                    </p>
                </div>
                <button
            ...
            @if (facade.isLoading()) {
                ...
                >
                    Chargement des signalements rejetés…
                </div>
            } @else if (hasError()) {
                <div
