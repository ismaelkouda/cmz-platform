...
export type ReportStatesListVolet =
    'approve' | 'evaluate' | 'close' | 'reject' | 'download';

/** Tooltip export — `totalCount` = total filtré backend (meta pagination), pas la page courante. */
export function reportStatesListExportTooltip(
...
export function reportStatesListExportDisabled(
    canExport: boolean,
    totalCount: number,
    loading: boolean,
    exporting: boolean
 ...
 */
export async function exportReportStatesList<TRow>(params: {
    excelExport: ExcelExportPort;
    notification: NotificationPort;
    translate: (key: string) => string;
    ns: string;
    volet: ReportStatesListVolet;
    canExport: boolean;
    totalCount: number;
    loading: boolean;
    exporting: boolean;
    ...
    const tooltip = reportStatesListExportTooltip(
        params.translate,
        params.ns,
        params.canExport,
        params.totalCount
    ...
    try {
        ...
        await exportTableToExcel(params.excelExport, {
            fileName: `cmz-report-states-${params.volet}`,
