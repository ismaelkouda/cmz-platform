...
import { OPTICAL_FIBER_NETWORK_FORM } from '../constants/optical-fiber-network-paths.constant';
import { OPTICAL_FIBER_NETWORK_TABLE } from '../constants/optical-fiber-network-table.constant';
...
/**
 * Liste `optical-fiber-network` — même tranche verticale que `SiteGroupList`/
 * `MobileNetworkList` (`cmz-filter` + `cmz-table` + `cmz-pagination` sur
 * `rxResource`).
 */
@Component({
...
export class OpticalFiberNetworkListComponent {
    ...
    private readonly store = inject(OpticalFiberNetworkFilterStore);
    ...
    protected readonly tableColumns = OPTICAL_FIBER_NETWORK_TABLE.cols;
    ...
    private readonly presenter = new OpticalFiberNetworkPresenter((k) =>
        this.i18n.translate(k)
    );

    protected readonly itemsVM = computed<OpticalFiberNetworkVmProps[]>(() => {
        const authorization = {
    ...
    protected readonly filterFields: Signal<FilterField[]> = computed(() => [
        {
            type: 'text',
            name: OPTICAL_FIBER_NETWORK_FILTER_KEYS.SEARCH,
            label: T + '.FILTER.SEARCH',
            placeholder: T + '.FILTER.SEARCH_PLACEHOLDER',
        },
        {
            type: 'select',
            name: OPTICAL_FIBER_NETWORK_FILTER_KEYS.OPERATOR,
            label: T + '.FILTER.OPERATOR',
            placeholder: 'COMMON.SELECT_PLACEHOLDER',
            options: Object.values(Operator).map((v) => ({
                label: v,
                value: v,
            })),
        },
        {
            type: 'date',
            name: OPTICAL_FIBER_NETWORK_FILTER_KEYS.START_DATE,
            label: T + '.FILTER.DATE.FROM',
        },
        {
            type: 'date',
            name: OPTICAL_FIBER_NETWORK_FILTER_KEYS.END_DATE,
    ...
    protected onAction(event: {
        row: OpticalFiberNetworkVmProps;
        actionId: string;
    }): void {
    ...
    private navigateToForm(
        ...
        void this.router.navigate(['../', OPTICAL_FIBER_NETWORK_FORM], {
