...
import { MOBILE_NETWORK_TABLE } from '../constants/mobile-network-table.constant';
...
/**
 * Liste `mobile-network` — même tranche verticale que `SiteGroupList`
 * (`cmz-filter` + `cmz-table` + `cmz-pagination` sur `rxResource`).
 */
@Component({
...
export class MobileNetworkListComponent {
    ...
    protected readonly tableColumns = MOBILE_NETWORK_TABLE.cols;
    ...
    private readonly presenter = new MobileNetworkPresenter((k) =>
        this.i18n.translate(k)
    );

    protected readonly itemsVM = computed<MobileNetworkVmProps[]>(() => {
        const authorization = {
    ...
    protected readonly filterFields: Signal<FilterField[]> = computed(() => [
        {
            type: 'text',
            name: MOBILE_NETWORK_FILTER_KEYS.SEARCH,
            label: T + '.FILTER.SEARCH',
            placeholder: T + '.FILTER.SEARCH_PLACEHOLDER',
        },
        {
            type: 'select',
            name: MOBILE_NETWORK_FILTER_KEYS.TECHNOLOGY,
            label: T + '.FILTER.TECHNOLOGY',
            placeholder: 'COMMON.SELECT_PLACEHOLDER',
            options: Object.values(Technology).map((v) => ({
                label: v,
                value: v,
            })),
        },
        {
            type: 'select',
            name: MOBILE_NETWORK_FILTER_KEYS.OPERATOR,
        ...
        {
            type: 'date',
            name: MOBILE_NETWORK_FILTER_KEYS.START_DATE,
            label: T + '.FILTER.DATE.FROM',
        },
        {
            type: 'date',
            name: MOBILE_NETWORK_FILTER_KEYS.END_DATE,
    ...
    private navigateToForm(
        ...
        void this.router.navigate(['../', MOBILE_NETWORK_FORM], {
