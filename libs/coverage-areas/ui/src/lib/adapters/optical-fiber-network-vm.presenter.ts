import { OpticalFiberNetworkEntity, Status } from '@cmz/coverage-areas-domain';
...
import { OpticalFiberNetworkVmProps } from './optical-fiber-network-vm-props.interface';

interface OpticalFiberNetworkPermission {
    authorization: {
        canEdit: boolean;
        canDelete: boolean;
        canEnable: boolean;
...
const T = 'COVERAGE_AREAS.OPTICAL_FIBER_NETWORK.TOOLTIP';

/**
 * Presenter (UI) : `OpticalFiberNetworkEntity` → view-model. Réutilise
 * `STATUS_LABEL`/`statusStyleOf` de `site-group` — même règle que
 * `mobile-network` (`Status`/`StatusStyle` partagés une fois par lib).
 */
export class OpticalFiberNetworkPresenter {
    ...
    map(
        ...
        permission: OpticalFiberNetworkPermission
