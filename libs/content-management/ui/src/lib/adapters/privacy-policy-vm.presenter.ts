...
interface PrivacyPolicyPermission {
    authorization: {
        canEdit: boolean;
        canDelete: boolean;
        canPublish: boolean;
...
const T = 'CONTENT_MANAGEMENT.PRIVACY_POLICY.TOOLTIP';
...
export class PrivacyPolicyPresenter {
    ...
    map(
        ...
        permission: PrivacyPolicyPermission
    ): PrivacyPolicyVmProps {
        const { authorization: can, tooltip } = permission;
        const isOff = item.status === PrivacyPolicyStatus.UNPUBLISH;
        ...
        return {
            ...
            status: item.status,
            statusLabel: this.t(PRIVACY_POLICY_STATUS_LABEL[item.status]),
            statusStyle: privacyPolicyStatusStyleOf(item.status),
