...
import type { TasksProcessingProps } from '@cmz/processing-domain';
import { TypeReport } from '@cmz/shared-domain';
import { TasksProcessingItemApiDto } from '../dtos/tasks-processing-response-api.dto';

@Service()
export class TasksProcessingItemMapper extends PaginatedMapper<
...
> {
    ...
    private readonly entityCache = new Map<string, TasksProcessingEntity>();

    protected override mapItemFromDto(
        dto: TasksProcessingItemApiDto
    ): TasksProcessingEntity {
        MapperUtils.validateDto(dto, { required: ['uniq_id'] });
        const props: TasksProcessingProps = {
        ...
        const entity = cached
            ? cached.with(props)
            : new TasksProcessingEntity(props);
