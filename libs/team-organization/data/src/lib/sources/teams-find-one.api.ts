...
import { TeamsFindOneResponseApiDto } from '../dtos/teams-find-one-response-api.dto';
...
export class TeamsFindOneApi {
    ...
    execute(
        dto?: TeamsFindOneFilterApiDto,
        options?: FetchOptions
    ): Observable<TeamsFindOneResponseApiDto> {
        const path = dto?.id ? `/${dto.id}` : '';
        const url = `${this.baseUrl}${TEAM_ORGANIZATION_ENDPOINTS.TEAMS}${path}`;
        const context = new HttpContext().set(
            BYPASS_CACHE,
            options?.forceRefresh ?? false
        );
        return this.http.get<TeamsFindOneResponseApiDto>(url, { context });
    }
}
