...
import { AllFinalizationItemMapper } from './all-finalization-item.mapper';
import type { AllFinalizationItemApiDto } from '../dtos/all-finalization-response-api.dto';

function makePaginatedResponse(items: AllFinalizationItemApiDto[]) {
    return {
        error: false,
        message: 'OK',
        data: {
...
function makeItemDto(
    partial: Partial<AllFinalizationItemApiDto> = {}
...
function createMapper(): AllFinalizationItemMapper {
    const injector = createEnvironmentInjector(
        [
            ReportTypeMapper,
            TelecomOperatorMapper,
            ReportSourceMapper,
            AllFinalizationItemMapper,
        ],
        null as never
    );
    return injector.get(AllFinalizationItemMapper);
}

describe('AllFinalizationItemMapper', () => {
    const mapper = createMapper();

    it('mappe le wire vers AllFinalizationEntity', () => {
