...
import type { TasksFinalizationProps } from '@cmz/finalization-domain';
import { TypeReport } from '@cmz/shared-domain';
import { TasksFinalizationItemApiDto } from '../dtos/tasks-finalization-response-api.dto';

@Service()
export class TasksFinalizationItemMapper extends PaginatedMapper<
...
> {
    ...
    private readonly entityCache = new Map<string, TasksFinalizationEntity>();

    protected override mapItemFromDto(
        dto: TasksFinalizationItemApiDto
    ): TasksFinalizationEntity {
        MapperUtils.validateDto(dto, { required: ['uniq_id'] });
        const props: TasksFinalizationProps = {
        ...
        const entity = cached
            ? cached.with(props)
            : new TasksFinalizationEntity(props);
