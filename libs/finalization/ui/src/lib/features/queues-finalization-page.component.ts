...
import { QueuesFinalizationFacade } from '@cmz/finalization-application';
import {
    NotificationPort,
...
import { QueuesFinalizationVmProps } from '../adapters/queues-finalization-vm-props.interface';
import { QueuesFinalizationPresenter } from '../adapters/queues-finalization-vm.presenter';
import { QUEUES_FINALIZATION_FILTER_KEYS } from '../constants/queues-finalization-filter-keys.constant';
import { FINALIZATION_QUEUES_ROUTE } from '../constants/finalization-paths.constant';
import { QUEUES_FINALIZATION_TABLE } from '../constants/queues-finalization-table.constant';
import { QueuesFinalizationFilterStore } from '../stores/queues-finalization-filter.store';
import {
...
@Component({
    selector: 'cmz-queues-finalization-page',
    imports: [
        FilterComponent,
        TableComponent,
        PaginationComponent,
    ...
    providers: [QueuesFinalizationFilterStore],
    template: `
...
export class QueuesFinalizationPageComponent {
    protected readonly facade = inject(QueuesFinalizationFacade);
    private readonly store = inject(QueuesFinalizationFilterStore);
    ...
    protected readonly tableColumns = QUEUES_FINALIZATION_TABLE.cols;
    protected readonly tableActions = QUEUES_FINALIZATION_TABLE.actions;

    /** Tranche B — branchement `FinalizationDetailsDialogComponent`. */
    ...
    private readonly presenter = new QueuesFinalizationPresenter((key) =>
        this.i18n.translate(key)
    );

    protected readonly itemsVM = computed<QueuesFinalizationVmProps[]>(() =>
        this.facade
            .items()
            .map((item) =>
    ...
    protected readonly filterFields: Signal<FilterField[]> = computed(() => [
        {
            type: 'text',
            name: QUEUES_FINALIZATION_FILTER_KEYS.INITIATOR_PHONE_NUMBER,
            label: `${T}.FILTER.INITIATOR`,
            placeholder: 'COMMON.PHONE_PLACEHOLDER',
        },
        {
            type: 'text',
            name: QUEUES_FINALIZATION_FILTER_KEYS.UNIQ_ID,
            label: `${T}.FILTER.UNIQ_ID`,
            placeholder: 'COMMON.REPORT_UNIQ_ID_PLACEHOLDER',
        },
        {
            type: 'select',
            name: QUEUES_FINALIZATION_FILTER_KEYS.REPORT_TYPE,
        ...
        {
            type: 'text',
            name: QUEUES_FINALIZATION_FILTER_KEYS.OPERATORS,
            label: `${T}.FILTER.OPERATORS`,
            placeholder: 'COMMON.SELECT_PLACEHOLDER',
        },
        {
            type: 'select',
            name: QUEUES_FINALIZATION_FILTER_KEYS.SOURCE,
        ...
        {
            type: 'date',
            name: QUEUES_FINALIZATION_FILTER_KEYS.START_DATE,
            label: 'COMMON.START_DATE',
        },
        {
            type: 'date',
            name: QUEUES_FINALIZATION_FILTER_KEYS.END_DATE,
