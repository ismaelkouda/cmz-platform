...
export type RequestsListVolet = 'queues' | 'tasks' | 'all';

/** Tooltip export — `totalCount` = total filtré backend (meta pagination), pas la page courante. */
export function finalizationListExportTooltip(
...
export function finalizationListExportDisabled(
    canExport: boolean,
    totalCount: number,
    loading: boolean,
    exporting: boolean
 ...
 */
export async function exportFinalizationList<TRow>(params: {
    excelExport: ExcelExportPort;
    notification: NotificationPort;
    translate: (key: string) => string;
    ns: string;
    volet: RequestsListVolet;
    canExport: boolean;
    totalCount: number;
    loading: boolean;
    exporting: boolean;
    ...
    const tooltip = finalizationListExportTooltip(
        params.translate,
        params.ns,
        params.canExport,
        params.totalCount
    ...
    try {
        ...
        await exportTableToExcel(params.excelExport, {
            fileName: `cmz-finalization-${params.volet}`,
