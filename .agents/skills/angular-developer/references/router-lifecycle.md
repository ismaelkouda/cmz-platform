...
Angular Router emits events through the `Router.events` observable, allowing you
to track the navigation lifecycle from start to finish.
...
4. **`ResolveStart` / `End`**: Data resolution phase (fetching data via
   resolvers).
...
Enable detailed console logging of all routing events during application
bootstrap.
...
- **Loading Indicators**: Show a spinner when `NavigationStart` fires and hide
  it on `NavigationEnd`/`Cancel`/`Error`.
