...
export function parseActionRequestMigrationArguments(arguments_) {
    const options = {};
...
export async function migrateActionRequestFile({
    definitionPath,
    backendContractPath,
    decisionsPath,
    outputPath,
...
function usage() {
    return 'Usage:\n  bun run migrate:action-request --definition <v1.json> --backend-contract <contract.json> --decisions <decisions.json> --out <v2.json>\n';
}

async function main() {
    const options = parseActionRequestMigrationArguments(process.argv.slice(2));
    if (options.help) {
    ...
    console.log(`action-request 2.0.0 written to ${result.output}`);
}
